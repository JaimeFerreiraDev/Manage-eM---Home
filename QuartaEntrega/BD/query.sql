...
update Kid, Kids_Task, Task set pts_Kid = pts_Kid + pts_Task where Kid.id_Kid=Kids_Task.kid and Task.id_Task=Kids_Task.Task;
 ...
 ## contar o numero de rewards pedidas pelos filhos de um pai
 select COUNT(Kids_Reward.reward)as num from Kids_Reward, User, Family_Relation where User.id_User = 1 
and User.id_User = Family_Relation.parent
and Family_Relation.kid = Kids_Reward.kid and Kids_Reward.requested = true;

 ## contar o numero de tasks acabadas pelos filhos de um pai
select COUNT(Kids_Task.Task)as num from Kids_Task, User, Family_Relation where User.id_User =1 
and User.id_User = Family_Relation.parent
 and Family_Relation.kid = Kids_Task.kid and Kids_Task.completed = true;
